...
export default function HomePage() {
	return (
		<main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden text-white">
			{/* 主渐变背景 */}
			<div className="absolute inset-0 bg-gradient-to-b from-slate-900 via-slate-700 to-slate-500" />

			{/* 协调色彩层次渐变 */}
			<div className="absolute inset-0 bg-gradient-to-b from-indigo-950/80 via-purple-900/60 to-pink-800/40" />
			...
			{/* 额外的纹理层，增强沙哑感 */}
			<div className="absolute inset-0 bg-gradient-to-br from-transparent via-white/5 to-white/10 mix-blend-overlay" />
			...
			<div className="container relative z-10 flex flex-col items-center justify-center gap-12 px-4 py-16">
				...
				<div className="w-full max-w-5xl">
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
						{features.map((feature) => (
							<div
								key={feature.title}
								className="flex flex-col gap-2 rounded-xl border border-white/20 bg-white/10 p-4 text-white backdrop-blur-sm"
							>
				...
				<div className="flex gap-4">
					<Link
						className="flex items-center justify-center rounded-xl border border-white/20 bg-white/10 px-6 py-3 font-semibold text-white no-underline backdrop-blur-sm transition hover:bg-white/20"
						href="https://zilin.im"
						target="_blank"
						rel="noopener noreferrer"
					>
